...
Phase 5（可选）— 语义检索
    ...
    • MySQL `symbols.embedding`（JSON 数组）；`npm run index` / `reindex` 在配置 `EMBEDDING_SERVICE_URL` 时对每条代码块写入向量（失败则跳过更新、保留旧向量）
...
**基于变更文件列表的局部更新**
    ...
    • 索引任务**仅解析、upsert 列表中的文件**；若文件已删除，则在 DB 中**删除对应代码块或标记失效**。

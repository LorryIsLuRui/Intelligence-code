 ...
 *
 * 策略：
 * - 只分析 changed files 中可索引的导出符号
 * - 与库内同 type 的存量符号做语义相似度（cosine）匹配
...
function getMetaArray(meta: Record<string, unknown> | null | undefined, key: string): string[] {
  if (!meta) return [];
  const v = meta[key];
...
function propsForRow(row: { type: SymbolType; meta: Record<string, unknown> }): string[] {
...
async function main() {
  ...
  const blockThreshold = Number(args.get("block-threshold") ?? process.env.DUPLICATE_BLOCK_THRESHOLD ?? "0.95");
  const warnThreshold = Number(args.get("warn-threshold") ?? process.env.DUPLICATE_WARN_THRESHOLD ?? "0.85");
  ...
  // 3) 对每个符号：拉同 type 候选（有 embedding），算 cosine，取 top1
  type DbCandidate = {
    id: number;
    name: string;
    type: SymbolType;
    ...
    for (const r of dbRows) {
      ...
      try {
        ...
        if (Array.isArray(parsed)) {
          ...
          if (nums.every((n: number) => Number.isFinite(n))) emb = nums;
        }
      } catch {
        emb = null;
      }
  ...
  for (let i = 0; i < rows.length; i++) {
    ...
    const oldProps = best.c.type === "component" ? uniqueLower(getMetaArray(best.c.meta, "props")) : [];
    const { coverage, isSuperset } = row.type === "component" ? coverageAndSuperset(newProps, oldProps) : { coverage: 1, isSuperset: true };

    const propsOk = row.type !== "component" ? true : coverage >= propsCoverageThreshold;
  ...
  const report = {
    ...
    extractedSymbols: rows.map((r) => ({ name: r.name, type: r.type, path: r.path })),
    findings
  };
  ...
  if (findings.length === 0) {
    ...
    lines.push("");
    lines.push("| 级别 | 新增/改动符号 | 类型 | 最相似存量 | 相似度 | props 超集 | props 覆盖率 |");
    lines.push("|---|---|---|---|---:|---:|---:|");

/**
 * 扫描源码目录，用 ts-morph 解析导出并生成待写入 MySQL 的符号行。
 */
...
/**
 * 将 MCP/ts 的 `default` 导出键还原为具体符号名（函数名、变量名、类名）。
...
/**
 * 按 glob 收集文件，用 ts-morph 加载并遍历每个文件的导出，生成全部符号行。
 * @returns 数组可交给 `upsertSymbols`；无匹配文件时返回空数组（不写库）。
 */
export async function indexProject(opts: IndexProjectOptions): Promise<IndexedSymbolRow[]> {

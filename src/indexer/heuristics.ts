 ...
 * @returns 命中`hook | ...`
 */
export function inferCategoryFromName(originName: string): string | null {
    const name = originName.toLowerCase();
    if (name.startsWith('use')) return 'hook';
    if (name.includes('fetch')) return 'network';
...
export function getLeadingDocDescription(node: Node): string | null {
    ...
    return (
        tryNode(node) ?? (node.getParent() ? tryNode(node.getParent()!) : null)
    );
}
 ...
 */
export function getRelativePathForDisplay(
    projectRoot: string,
    absolutePath: string
): string {
